...
// The cycle a given serving-order slot follows as the rotation number
// increases: 1 (serving) -> 6 -> 5 -> 4 -> 3 -> 2 -> back to 1.
const ZONE_ORDER = [1, 6, 5, 4, 3, 2];

/** Which zone a given serving-order slot (1-6) occupies during a given rotation number (1-6). */
export function zoneForSlot(rotationNum, slotNum) {
  const diff = ((rotationNum - slotNum) % 6 + 6) % 6;
  return ZONE_ORDER[diff];
}

/** Inverse of zoneForSlot: which slot (1-6) occupies a given zone during a given rotation. */
export function slotInZone(rotationNum, zone) {
  const diff = ZONE_ORDER.indexOf(zone);
  return ((rotationNum - diff - 1) % 6 + 6) % 6 + 1;
}
...
/** A libero's covered players - supports the old single-target shape
 * ({forPlayerId}) and the current multi-target shape ({forPlayerIds}). */
export function liberoTargets(libero) {
  if (Array.isArray(libero.forPlayerIds)) return libero.forPlayerIds;
  return libero.forPlayerId ? [libero.forPlayerId] : [];
}

/** Which one of a libero's targets (if any) they're authorized to serve for.
 * Supports the old {canServe} boolean (paired with a single forPlayerId) and
 * the current {servesForPlayerId} shape. */
export function liberoServesFor(libero) {
  if (libero.servesForPlayerId !== undefined) return libero.servesForPlayerId;
  if (libero.canServe && libero.forPlayerId) return libero.forPlayerId;
  return null;
}

/**
 * Resolves, for one rotation, which player actually occupies each of the 6
 * zones - accounting for the starting lineup, any active libero coverage,
 * planned substitutions, and substitution serving rights.
 *
